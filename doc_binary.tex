...
precession, couple the spins, and radiate orbital energy.  We provide the
\texttt{post\_newtonian} effect, which implements the harmonic‐coordinate
point‐mass equations of motion of \citet{Kidder1995} for every massive pair:
\begin{itemize}[nosep,leftmargin=1.8em]
  \item $1$\,PN conservative periapsis‐precession terms;
  \item $1.5$\,PN spin–orbit couplings;
  \item $2$\,PN point‐mass and spin–spin conservative corrections; and
  \item $2.5$\,PN gravitational‐wave radiation reaction.
\end{itemize}

For two bodies of mass $m_i$ and $m_j$ separated by $\mathbf{r}$ with
relative velocity $\mathbf{v}$, let
\[
  m = m_i + m_j, \quad
...
\]
Then the added relative acceleration is
\[
  \mathbf{a}_{\rm 1PN} =
    -\frac{G m}{r^2 c^2}\Bigl[
       \mathbf{n}\bigl((1+3\eta)v^2 - 2(2+\eta)\tfrac{Gm}{r} - \tfrac{3}{2}\eta\,\dot r^2\bigr)
     \Bigr]
    + \frac{G m}{r^2 c^2}(4-2\eta)\,\dot r\,\mathbf{v},
\]
\[
  \mathbf{a}_{\rm 2.5PN} =
    \frac{8\,G^2 m^2\,\eta}{5\,c^5\,r^3}\Bigl[
      \dot r\Bigl(18\,v^2 + \tfrac{2}{3}\,\tfrac{G m}{r} - 25\,\dot r^2\Bigr)\mathbf{n}
     -\Bigl(6\,v^2 - 2\,\tfrac{G m}{r} - 15\,\dot r^2\Bigr)\mathbf{v}
    \Bigr].
\]
The $1.5$\,PN (spin–orbit) and $2$\,PN (point‐mass plus spin–spin) pieces
are implemented exactly as Eqs.~2.2c–e of \citet{Kidder1995} and require each
particle’s spin angular momentum vector, supplied via
\texttt{pn\_spin}.  After computing the pairwise $\mathbf{a}$, it is
distributed to the two bodies in proportion to their masses,
so that total linear momentum is conserved.

\medskip
\noindent
The only required effect parameter is
\[
  \texttt{c}\quad\bigl[\text{length}/\text{time}\bigr],
\]
the speed of light in simulation units.  Four optional integer flags
toggle each PN order (all default to 1):
\[
  \texttt{pn\_1PN},\;
  \texttt{pn\_15PN},\;
  \texttt{pn\_2PN},\;
...
\texttt{pn\_2PN}     & —                 & Include $2$\,PN point‐mass and spin–spin (default: 1)      \\
\texttt{pn\_25PN}    & —                 & Include $2.5$\,PN radiation reaction (default: 1)          \\
\bottomrule
\end{tabular}
\end{table}
...
Setting $c$ (and optionally the four \texttt{pn\_*} flags) to unity, or
to the desired ratio of code units to physical units, reproduces the
classical tests of perihelion advance and gravitational‐wave inspiral
