...
\mathbf{a}_{2.5\mathrm{PN}} &= \frac{8 G^2 m^2 \eta}{5 c^5 r^3}\Bigl[\dot r\Bigl(3\frac{G m}{r}+v^2\Bigr)\mathbf{n}-\Bigl(\frac{G m}{r}+v^2\Bigr)\mathbf{v}\Bigr],
\end{align}
where $m=m_i+m_j$, $\eta=m_i m_j/m^2$, $\mathbf{n}=\mathbf{r}/r$, and
$\dot r = \mathbf{v}\cdot\mathbf{n}$. The relative acceleration is distributed
to the two particles in proportion to their masses, preserving linear momentum.
